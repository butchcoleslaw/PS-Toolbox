...
#>
    ...
    Switch ($PsCmdlet.ParameterSetName)
    {
       ...
       "set2" # Body
       {
           ...
           switch ($Category)
           {
               INF  { $Message = ("$date{0} INF{0} $Message" -f $Delimiter); $Color = 'Cyan'   ; break }
               WAR  { $Message = ("$date{0} WAR{0} $Message" -f $Delimiter); $Color = 'Yellow' ; break }
               ERR  { $Message = ("$date{0} ERR{0} $Message" -f $Delimiter); $Color = 'Red'    ; break }
           }
